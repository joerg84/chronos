  ...
  def buildMesosCredentials(principal: String, secretFile: Option[String]): Credential = {
    ...
    secretFile foreach { file =>
      try {
        ...
        val filePermissions = PosixFilePermissions.toString(Files.getPosixFilePermissions(Paths.get(file)))
        (new Regex("^[-rwe]{6}---$")).findFirstIn(filePermissions).getOrElse(log.warning(s"Secret file $file has global permissions."))
